...
if [[ -z "${tag}" ]]; then
  echo "ERROR: missing X07_TOOLCHAIN_TAG" >&2
  exit 2
fi

version="${tag#v}"
if [[ -z "${version}" || "${version}" == "${tag}" ]]; then
  echo "ERROR: X07_TOOLCHAIN_TAG must be like vX.Y.Z (got: ${tag})" >&2
  exit 2
fi
...
tarball="${X07_TOOLCHAIN_TARBALL:-}"
if [[ -z "${tarball}" ]]; then
  case "${platform}-${arch}" in
    Linux-x86_64) tarball="x07-${version}-x86_64-unknown-linux-gnu.tar.gz" ;;
    Linux-aarch64) tarball="x07-${version}-aarch64-unknown-linux-gnu.tar.gz" ;;
    Darwin-x86_64) tarball="x07-${version}-x86_64-apple-darwin.tar.gz" ;;
    Darwin-arm64) tarball="x07-${version}-aarch64-apple-darwin.tar.gz" ;;
    *)
      echo "ERROR: unsupported platform/arch for published x07 toolchains: ${platform}-${arch}" >&2
      echo "NOTE: on Windows, install and run inside WSL2." >&2
      exit 2
      ;;
  esac
fi
...
if [[ -x "${x07_bin}" ]]; then
  if "${x07_bin}" --version | grep -Eq "x07 ${version}$"; then
    exit 0
  fi
fi
